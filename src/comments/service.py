...
async def create_comment(photo_id: int, comment: CommentCreate,
                                 db: AsyncSession = Depends(get_db), current_user: User = Depends(get_current_user)) -> CommentModel:
    """
...
async def get_comments(photo_id:int, db: AsyncSession = Depends(get_db))->List[CommentModel]:
    """
    ...
    try:
        ...
        return CommentModel(status="ok",message=" ", data=[comment for comment in comments])
    except Exception as e:
...
async def update_comment(comment_id: int, comment:CommentUpdate,
                         ...
                         current_user : User = Depends(get_current_user))->CommentModel:
    """
    ...
    try:
        result = await db.execute(select(Comment).
                                  filter(and_(Comment.id == comment_id, Comment.user_id == current_user.id)))
